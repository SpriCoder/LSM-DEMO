...
public class BPlusNode<K extends Comparable<K>, V> {
  ...
  protected void updateRemove(BPlusTree<K, V> tree) {
    // if the number of child nodes is less than order/2 or less than 2, you need to merge nodes
    if (children.size() < tree.getOrder() / 2 || children.size() < 2) {
      ...
      // If the number of child nodes of the previous node is greater than order/2 and greater than
      // 2,
      // the borrow node
      if (previous != null
      ...
      // If the number of child nodes of the post node is greater than order/2 and greater than 2,
      // then borrow node
  ...
  public V remove(K key, BPlusTree<K, V> tree) {
    ...
    if (isLeaf) {
      ...
      // if the size of records of current node greater than order / 2, then delete directly
      if (records.size() > tree.getOrder() / 2 && records.size() > 2) {
        return remove(key);
      }
      // if the size of records of current node less than order / 2 and the size of records
      // of previous node greater than order / 2, then borrow node
      if (previous != null
      ...
      // if the size of records of current node less than order / 2 and  the size of records
      // of next node greater than order / 2, then borrow node
      if (next != null
          && next.parent == parent
